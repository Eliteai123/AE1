...
export function MenuContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative w-[56px] sm:w-[64px]" data-expanded={isExpanded}>
      {/* Container for all items */}
      <div className="relative">
        {/* First item - always visible */}
        <div 
          className="relative w-14 h-14 sm:w-16 sm:h-16 bg-white/5 backdrop-blur-lg cursor-pointer rounded-full group will-change-transform z-50"
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-14 h-14 sm:w-16 sm:h-16 bg-white/5 backdrop-blur-lg will-change-transform"
            style={{
